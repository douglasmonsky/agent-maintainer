...
- Pull-request and release verification now fan out verifier-owned groups and
  aggregate exact-commit partial manifests before the protected final result;
...
- Local commits now run fast staged policy plus affected tests, while pre-push
  retains the complete precommit profile. Unprofiled dogfood timings fell from
  roughly 31 seconds for the prior full-suite hook to 2.08 seconds for docs,
  2.21 seconds for tests, 6.97 seconds for source plus its required test, and
  22.59 seconds for the broad 21-file cross-package batch.
