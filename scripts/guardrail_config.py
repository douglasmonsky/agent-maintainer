...
- `guardrail_lib.config.schema` owns constants and the dataclass.
- `guardrail_lib.config.coercion` owns value coercion.
- `guardrail_lib.config.modes` owns built-in presets.
- `guardrail_lib.config.loader` owns pyproject and environment loading.
"""
...
from guardrail_lib.config import loader, modes, schema
