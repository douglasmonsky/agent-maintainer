...
def write_hook_event(
    ...
    (log_dir / maintainer_doctor_hook_audit.HOOK_AUDIT_NAME).write_text(
        json.dumps(
            {
                "timestamp": timestamp,
                "hook": "PostToolUse",
...
def test_hook_audit_warns_when_enabled_without_records(tmp_path: Path) -> None:
    ...
    result = maintainer_doctor_hook_audit.check_hook_audit(
        tmp_path,
        MaintainerConfig(diagnostic_artifacts_dir=".custom-logs"),
    )
...
def test_valid_hook_events_reports_unreadable_audit_path(tmp_path: Path) -> None:
    events, malformed_count = maintainer_doctor_hook_audit.valid_hook_events(tmp_path)
...
def test_hook_audit_reports_recent_valid_event_with_malformed_count(
    ...
    log_dir = tmp_path / ".verify-logs"
    log_dir.mkdir()
    audit_path = log_dir / maintainer_doctor_hook_audit.HOOK_AUDIT_NAME
...
def test_event_timestamp_rejects_non_string_timestamp() -> None:
    ...
    assert maintainer_doctor_hook_audit.event_timestamp(event) is None
...
def test_event_timestamp_accepts_naive_timestamp() -> None:
    ...
    parsed = maintainer_doctor_hook_audit.event_timestamp(event)
...
def test_no_valid_events_result_without_malformed_count() -> None:
    result = maintainer_doctor_hook_audit.no_valid_events_result(0)
...
def test_hook_audit_result_defensively_handles_invalid_event_timestamp() -> None:
    ...
    result = maintainer_doctor_hook_audit.hook_audit_result(event)
