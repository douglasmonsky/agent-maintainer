...
def test_runtime_events_record_run_artifacts(
    ...
    runtime_eventing.run_fresh(events, log_dir=tmp_path / "runs" / "run-1")
    runtime_eventing.run_reused(events, exit_code=0, log_dir=tmp_path)
...
def event_attributes(record: dict[str, object]) -> dict[str, int]:
    """Return typed event attributes."""

    return cast("dict[str, int]", record["attributes"])
