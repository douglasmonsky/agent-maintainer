...
def check_recent_logs(
    repo_root: Path, config: maintainer_config.MaintainerConfig | None = None
) -> DoctorResult:
    ...
    if issues:
        return DoctorResult(
            ...
            hint="Rerun the relevant maintainer verify profile.",
        )
...
def catalog_drift_issues(
    config: maintainer_config.MaintainerConfig, payload: dict[str, object]
) -> list[str]:
