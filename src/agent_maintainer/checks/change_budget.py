...
Source and test roots are configurable through [tool.agent_maintainer], environment
...
def diff_target_label(base_ref: str, *, staged: bool) -> str:
    """Return diff target label in error messages."""

    return "staged changes" if staged else repr(base_ref)


def copied_source_paths(base_ref: str, *, staged: bool) -> frozenset[str]:
    """Return source paths whose removed content is copied elsewhere."""
    ...
    try:
        ...
        raise RuntimeError(f"Could not calculate diff names against {target}: {stderr}") from exc
...
def run_git_numstat(base_ref: str, *, staged: bool) -> list[FileChange]:
    ...
    copied_sources = copied_source_paths(base_ref, staged=staged)
    ...
    for line in result.stdout.splitlines():
        ...
        changes.append(FileChange(path=path, added=int(added_raw), deleted=deleted))
    return changes


def changed_python_files(
    changes: list[FileChange], config: MaintainerConfig
