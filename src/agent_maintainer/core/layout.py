...
from agent_maintainer.core.config import MaintainerConfig, any_path_exists, format_paths
...
def layout_failures(config: MaintainerConfig, profile: str) -> list[str]:
    ...
    failures = [
        configured_path_failure(
            "source root",
            config.source_roots,
            "Set [tool.agent_maintainer].source_roots, "
            "AGENT_MAINTAINER_SOURCE_ROOTS, or --source-root.",
        ),
        configured_path_failure(
...
def test_layout_failures(config: MaintainerConfig) -> list[str | None]:
