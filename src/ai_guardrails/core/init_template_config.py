...
STARTER_PYPROJECT = textwrap.dedent(
    """\
    # Starter guardrail config for package-first adoption.
    ...
    # Core paths. Package users should not vendor src/ai_guardrails into their app.
    source_roots = ["src"]
    test_roots = ["tests"]
    package_paths = ["src"]
    coverage_source = ["src"]
    file_length_paths = ["src", "tests", ".codex/hooks"]
    structure_paths = ["src", ".codex/hooks"]
