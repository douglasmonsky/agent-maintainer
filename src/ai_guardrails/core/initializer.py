"""Initialize package-first guardrail adoption files."""
...
from ai_guardrails.core.init_templates import (
    CORE_TRACK,
    STARTER_FILES,
    TRACKS,
    StarterFile,
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(prog="python -m ai_guardrails init")
    parser.add_argument(
        "--target",
        type=Path,
        default=Path.cwd(),
...
def main(argv: list[str]) -> int:
    """Write starter guardrail files for a package-first install."""
