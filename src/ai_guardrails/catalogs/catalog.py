...
from ai_guardrails.catalogs import python as python_checks
from ai_guardrails.catalogs.docs import docs_config_checks
from ai_guardrails.catalogs.security import (
...
from ai_guardrails.config.schema import (
    FRESH_STRICT_MODE,
    IMPORT_LINTER_TOOL,
    TACH_TOOL,
    GuardrailConfig,
)
from ai_guardrails.core.config import existing_paths
...
def tach_checks(config: GuardrailConfig) -> list[models.Check]:
    ...
    config_command = [sys.executable, "-m", "ai_guardrails.checks.tach_config"]
    if strict:
...
def make_checks(
    ...
    return [
        models.Check(
            ...
            models.ALL_PROFILES,
            required_paths=("src/ai_guardrails/checks/file_lengths.py",),
        ),
        models.Check(
            "structure-cohesion",
            structure_command(config),
            models.ALL_PROFILES,
            required_paths=("src/ai_guardrails/checks/structure.py",),
        ...
        *change_budget_checks(config, base_ref, staged=staged),
        models.Check(
            ...
            models.ALL_PROFILES,
            required_paths=("src/ai_guardrails/checks/suppression_budget.py", ".git"),
        ),
        models.Check(
            "ruff-format",
            ["ruff", "format", "--check", "."],
...
def change_budget_checks(
    ...
    return [
        models.Check(
            ...
            frozenset((profile,)),
            required_paths=("src/ai_guardrails/checks/change_budget.py", ".git"),
            report_success_output=True,
        )
...
def file_length_command(config: GuardrailConfig, file_length_paths: tuple[str, ...]) -> list[str]:
    ...
    command = [sys.executable, "-m", "ai_guardrails.checks.file_lengths", *file_length_paths]
...
def structure_command(config: GuardrailConfig) -> list[str]:
    ...
    command = [
        sys.executable,
        "-m",
        "ai_guardrails.checks.structure",
        "--warn-threshold",
...
def change_budget_command(
    ...
    *,
    ...
    command = [sys.executable, "-m", "ai_guardrails.checks.change_budget", base_ref]
    if staged:
...
def suppression_budget_command(base_ref: str, *, staged: bool) -> list[str]:
    ...
    command = [sys.executable, "-m", "ai_guardrails.checks.suppression_budget", base_ref]
    if staged:
        command.append("--staged")
    return command
