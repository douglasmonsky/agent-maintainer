"""Python quality and security checks for the guardrail catalog."""
...
from ai_guardrails.config.schema import FRESH_STRICT_MODE, GuardrailConfig
...
def pip_audit_check(config: GuardrailConfig) -> models.Check:
    ...
    if not config.pip_audit_args:
        if config.mode == FRESH_STRICT_MODE:
            return models.Check(
                "pip-audit",
                [sys.executable, "-m", "ai_guardrails.checks.pip_audit_config"],
                models.FULL_PROFILES,
                required_paths=("src/ai_guardrails/checks/pip_audit_config.py",),
            )
        return models.Check(
            "pip-audit",
            ["pip-audit"],
...
def mutmut_check(config: GuardrailConfig) -> models.Check:
    ...
    command = [sys.executable, "-m", "ai_guardrails.runners.mutmut", *config.mutmut_args]
    ...
    return models.Check(
        ...
        command,
        models.MANUAL_PROFILES,
        required_paths=("src/ai_guardrails/runners/mutmut.py",),
...
def pyright_check(config: GuardrailConfig) -> models.Check:
    ...
    return models.Check(
        "pyright",
        [sys.executable, "-m", "ai_guardrails.runners.pyright"],
        models.LOCAL_GATE_PROFILES,
        required_paths=("src/ai_guardrails/runners/pyright.py",),
...
def ruff_check(config: GuardrailConfig) -> models.Check:
    ...
    return models.Check(
        "ruff",
        [sys.executable, "-m", "ai_guardrails.runners.ruff"],
        models.ALL_PROFILES,
        required_paths=("src/ai_guardrails/runners/ruff.py",),
        required_executable="ruff",
        artifact_paths=(str(artifacts_dir / "ruff.json"),),
    )


def bandit_check(config: GuardrailConfig) -> models.Check:
    """Build the Bandit check through the JSON-artifact wrapper."""

    artifacts_dir = Path(config.diagnostic_artifacts_dir)
    return models.Check(
        "bandit",
        [sys.executable, "-m", "ai_guardrails.runners.bandit"],
        models.FULL_PROFILES,
        required_paths=("src/ai_guardrails/runners/bandit.py",),
