...
Hints look for repeated prefixes such as `maintainer_` and `check_`, repeated
...
already uses that shape in `src/agent_maintainer/config/`: schema, loading, coercion,
