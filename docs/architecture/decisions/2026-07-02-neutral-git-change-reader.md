...
`assess reviewability` provides advisory provider-aware reporting. It needs to
see changed files across enabled ecosystems before any Python-specific
reviewability policy is applied.
...
Reusing that helper for advisory TypeScript/JavaScript reviewability created a
hidden coupling: non-Python dependency files such as `package-lock.json`,
`pnpm-lock.yaml`, and `yarn.lock` could be filtered before provider
classification saw them.
...
without Python path exclusions. `assess.reviewability` now depends on the
neutral reader instead of `checks.change_budget`.

The blocking Python `change-budget` gate keeps its existing helper, exclusions,
wording, and behavior.

## Consequences

- TypeScript/JavaScript lockfiles stay visible in `assess reviewability`.
...
- Add TypeScript-specific git readers. Rejected because the git observation step
  is ecosystem-neutral; providers should classify paths after collection.
